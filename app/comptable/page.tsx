...
const MOIS = ["01 - Janvier", "02 - Février", "03 - Mars", "04 - Avril", "05 - Mai", "06 - Juin",
...
export default function Comptable() {
  ...
  async function charger(tk: string, an: string, mo: string, rech: string) {
    setChargement(true);
  ...
  function appliquerFiltres() {
    setDetailId(null);
    charger(token, annee, mois, recherche.trim());
  }
        ...
        <div style={carte}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "flex-end" }}>
            ...
            {(annee || mois || recherche) && (
              <button style={lien} onClick={() => { setAnnee(""); setMois(""); setRecherche(""); charger(token, "", "", ""); }}>Réinitialiser</button>
            )}
          </div>
        </div>
